import { z } from "zod";
import { optionalPublicExternalUrl } from "@/lib/utils";

const optionalUrl = optionalPublicExternalUrl();

const optionalDate = z.union([z.literal(""), z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Informe uma data válida.")]);
...
export const projectFormSchema = z
    .object({
        ...
        titlePt: z.string().trim().min(2, "Informe o título em português.").max(191),
        summaryPt: z.string().trim().min(10, "O resumo em português deve ter pelo menos 10 caracteres."),
        ...
        summaryEn: z.string().trim().min(10, "O resumo em inglês deve ter pelo menos 10 caracteres."),
