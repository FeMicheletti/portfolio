...
export function loginRateLimitKey(email: string) {
    const secret = process.env.LOGIN_RATE_LIMIT_SECRET || "portfolio-login-rate-limit";
    return createHash("sha256").update(`${secret}:${email.toLowerCase()}`).digest("hex");
}
...
export async function registerFailedLogin(key: string) {
    const now = new Date();
    const record = await prisma.loginRateLimit.findUnique({ where: { key } });
    const expired = !record || now.getTime() - record.windowStart.getTime() > WINDOW_MS;
