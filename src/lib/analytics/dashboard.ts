...
export async function getAnalyticsDashboard(period: AnalyticsPeriod) {
  ...
  const projects = new Map<
    string,
    { name: string; views: number; demos: number; repositories: number }
  >();
  ...
  for (const event of events) {
    ...
    if (interactionEvents.has(event.eventType))
    ...
    if (event.eventType === "PAGE_VIEW") {
      ...
      const source = event.utmSource
        ? `Campanha: ${event.utmSource}`
        : event.referrerHost || "Acesso direto";
  ...
  const clicks = events.filter((event) =>
    interactionEvents.has(event.eventType),
  ).length;
  ...
  return {
    ...
    clicks,
    engagementRate: sessions.size
      ? Math.round((engagedSessions.size / sessions.size) * 100)
      : 0,
    ...
    devices: Array.from(devices, ([name, value]) => ({ name, value })).sort(
      (a, b) => b.value - a.value,
    ),
    ...
    projects: Array.from(projects.values()).sort(
      (a, b) =>
        ...
        b.demos +
        b.repositories -
        (a.views + a.demos + a.repositories),
    ),
  };
}
