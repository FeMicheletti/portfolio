...
export function TrafficTrendChart({ data }: { data: Array<{ date: string; views: number; visitors: number }> }) {
    return (
        <ChartContainer config={trendConfig} className="h-72 w-full aspect-auto">
            <AreaChart data={data} margin={{ left: -20, right: 8, top: 8 }}>
                ...
                <ChartTooltip content={<ChartTooltipContent className="border-white/10 bg-zinc-950 text-zinc-200" />} />
                <Area type="monotone" dataKey="views" stroke="var(--color-views)" fill="url(#views-fill)" strokeWidth={2} />
...
export function DistributionChart({ data }: { data: Array<{ name: string; value: number }> }) {
    return (
        <ChartContainer config={distributionConfig} className="h-64 w-full aspect-auto">
            <BarChart data={data} layout="vertical" margin={{ left: 8, right: 8 }}>
                ...
                <ChartTooltip content={<ChartTooltipContent hideLabel className="border-white/10 bg-zinc-950 text-zinc-200" />} />
