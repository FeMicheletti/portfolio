...
export function AdminShell({ admin, children }: AdminShellProps) {
	...
	return (
		<div className="dark scheme-dark min-h-screen bg-zinc-950 text-zinc-100">
			{mobileMenuOpen ? <button type="button" aria-label="Fechar menu" className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm lg:hidden" onClick={() => setMobileMenuOpen(false)} /> : null}

			<aside className={cn("fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-violet-500/15 bg-zinc-950/95 shadow-2xl shadow-violet-950/20 backdrop-blur-xl transition-transform duration-200 lg:translate-x-0", mobileMenuOpen ? "translate-x-0" : "-translate-x-full")}>
				<div className="flex h-16 items-center justify-between border-b border-white/5 px-5">
					...
					<Button type="button" variant="ghost" size="icon-sm" className="text-zinc-400 hover:bg-white/5 hover:text-white lg:hidden" onClick={() => setMobileMenuOpen(false)}>
						<X className="size-5" />
				...
				<nav className="flex-1 space-y-1 overflow-y-auto px-3 py-5">
					...
					{navigation.map((item) => {
						...
						if (!item.available) {
							return (
								<div key={item.href} aria-disabled="true" className="flex h-10 cursor-not-allowed items-center gap-3 rounded-lg px-3 text-sm text-zinc-600">
						...
						return (
							<Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)} className={cn("flex h-10 items-center gap-3 rounded-lg px-3 text-sm font-medium transition-colors", active ? "bg-violet-600 text-white shadow-lg shadow-violet-950/30" : "text-zinc-400 hover:bg-white/5 hover:text-zinc-100")}>
								<Icon className="size-4" />
								{item.label}
							</Link>
						);
				...
				<div className="border-t border-white/5 p-3">
					<div className="mb-2 flex items-center gap-3 rounded-lg bg-white/2.5 p-3">
						<div className="flex size-9 shrink-0 items-center justify-center rounded-full bg-violet-500/15 text-sm font-semibold text-violet-300 ring-1 ring-violet-500/20">{admin.name.charAt(0).toUpperCase()}</div>
			...
			<div className="min-h-screen lg:pl-72">
				<header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-white/5 bg-zinc-950/80 px-4 backdrop-blur-xl sm:px-6">
					<div className="flex items-center gap-3">
						<Button type="button" variant="ghost" size="icon" className="text-zinc-400 hover:bg-white/5 hover:text-white lg:hidden" onClick={() => setMobileMenuOpen(true)}>
