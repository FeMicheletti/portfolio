...
function TranslationFields({ locale, formId, values, state, onChange }: { locale: "Pt" | "En"; formId: string; values?: ExperienceFormValues; state: ExperienceFormState, onChange: (name: keyof ExperienceFormValues, value: string | number | boolean) => void; }) {
	...
	return (
		<div className="space-y-4 pt-4">
			<div className="space-y-2">
				...
				<Input id={titleId} name={titleKey} value={values?.[titleKey]} onChange={(event) => onChange(titleKey, event.target.value)} placeholder="Senior Full-Stack Developer" className={controlClass} aria-invalid={Boolean(state.fieldErrors?.[titleKey])} />
				<FieldError errors={state.fieldErrors?.[titleKey]} />
			</div>
			<div className="space-y-2">
				<Label htmlFor={summaryId}>Resumo curto</Label>
				<Textarea id={summaryId} name={summaryKey} value={values?.[summaryKey]} onChange={(event) => onChange(summaryKey, event.target.value)} maxLength={500} rows={3} className={controlClass} aria-invalid={Boolean(state.fieldErrors?.[summaryKey])} />
				<FieldError errors={state.fieldErrors?.[summaryKey]} />
			</div>
			<div className="space-y-2">
				<Label htmlFor={descriptionId}>Como foi essa experiência</Label>
				<Textarea id={descriptionId} name={descriptionKey} value={values?.[descriptionKey]} onChange={(event) => onChange(descriptionKey, event.target.value)} rows={8} placeholder="Responsabilidades, desafios, tecnologias, resultados e aprendizados..." className={controlClass} aria-invalid={Boolean(state.fieldErrors?.[descriptionKey])} />
	...
	return (
		<form action={formAction} className="space-y-5">
			{state.error || state.success ? (
				<div className={`flex items-start gap-2 rounded-lg border px-3 py-2 text-xs ${state.success ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-300" : "border-red-500/20 bg-red-500/10 text-red-300"}`}>
						...
						<CardContent>
							<div className="grid gap-4 sm:grid-cols-2 mb-5">
								<div className="space-y-2">
									...
									<Input id={`company-${id ?? "new"}`} name="company" value={formValues.company} onChange={(event) => setField("company", event.target.value)} placeholder="Empresa" className={controlClass} aria-invalid={Boolean(state.fieldErrors?.company)} />
									<FieldError errors={state.fieldErrors?.company} />
								</div>
								<div className="space-y-2">
									<Label htmlFor={`location-${id ?? "new"}`}>Localização</Label>
									<Input id={`location-${id ?? "new"}`} name="location" value={formValues.location} onChange={(event) => setField("location", event.target.value)} placeholder="Rio de Janeiro, Brasil · Remoto" className={controlClass} />
								</div>
							</div>
							...
							<div className="mb-5">
								<div className="">
									...
									<Input id={`companyUrl-${id ?? "new"}`} name="companyUrl" type="url" value={formValues.companyUrl} onChange={(event) => setField("companyUrl", event.target.value)} placeholder="https://empresa.com" className={controlClass} aria-invalid={Boolean(state.fieldErrors?.companyUrl)} />
							...
							<div className="grid gap-4 sm:grid-cols-2 mb-5">
								<div className="space-y-2">
									...
									<Input id={`startedAt-${id ?? "new"}`} name="startedAt" type="date" value={formValues.startedAt} onChange={(event) => setField("startedAt", event.target.value)} className={controlClass} aria-invalid={Boolean(state.fieldErrors?.startedAt)} />
									<FieldError errors={state.fieldErrors?.startedAt} />
								</div>
								<div className="space-y-2">
									<Label htmlFor={`finishedAt-${id ?? "new"}`}>Data de término</Label>
									<Input id={`finishedAt-${id ?? "new"}`} name="finishedAt" type="date" value={formValues.finishedAt} onChange={(event) => setField("finishedAt", event.target.value)} className={controlClass} aria-invalid={Boolean(state.fieldErrors?.finishedAt)} />
							...
							<div className="flex flex-wrap gap-6 mb-5">
								<label className="flex items-center gap-3 text-sm text-zinc-300">
									<input name="current" type="checkbox" checked={formValues.current} onChange={(event) => setField("current", event.target.checked)} className="size-4 accent-violet-600" />
									Emprego atual
								</label>
								<label className="flex items-center gap-3 text-sm text-zinc-300">
									<input name="visible" type="checkbox" checked={formValues.visible} onChange={(event) => setField("visible", event.target.checked)} className="size-4 accent-violet-600" />
