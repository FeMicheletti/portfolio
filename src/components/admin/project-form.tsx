...
export function ProjectForm({
    ...
    values: ProjectFormValues;
    technologyCategories: TechnologyCategoryOption[];
    ...
    function toggleTechnology(technologyId: string, checked: boolean) {
        setFormValues((current) => {
            ...
            return {
                ...current,
                technologyIds: checked ? [...selected, technologyId] : selected.filter((id) => id !== technologyId),
            };
        });
    }
                ...
                <div className="space-y-6">
                    <Card className="border-violet-500/10 bg-zinc-900/70 shadow-lg shadow-black/10 ring-0">
                        ...
                        <CardContent className="space-y-5">
                            ...
                            <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-white/5 bg-white/2.5 p-3">
                                ...
                                />
                                <span>
                                    <span className="block text-sm font-medium text-zinc-200">Projeto em destaque</span>
                                    <span className="mt-0.5 block text-xs leading-5 text-zinc-500">Receberá maior evidência no portfólio.</span>
                                </span>
                            </label>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="border-violet-500/10 bg-zinc-900/70 shadow-lg shadow-black/10 ring-0">
                        <CardHeader className="border-b border-white/5">
                            ...
                            <CardDescription className="text-zinc-500">Selecione as tecnologias utilizadas neste projeto.</CardDescription>
                        </CardHeader>
                                ...
                                <Accordion type="multiple" className="space-y-2">
                                    {technologyCategories.map((category) => {
                                        const selectedCount = category.technologies.filter((technology) =>
                                            formValues.technologyIds.includes(technology.id),
                                        ).length;
                                        ...
                                        return (
                                            <AccordionItem
                                                key={category.id}
                                                value={category.id}
                                                className="overflow-hidden rounded-lg border border-white/5 bg-white/2.5 px-3"
                                            >
                                                <AccordionTrigger className="py-3 hover:no-underline">
                                                    <div className="flex min-w-0 flex-1 flex-col gap-2 pr-2 sm:flex-row sm:items-center sm:justify-between">
                                                        <div className="min-w-0">
                                                            <p className="truncate text-sm font-medium text-zinc-300">{category.namePt}</p>

                                                            {!category.visible ? (
                                                                <p className="mt-0.5 text-[10px] text-zinc-600 uppercase">Categoria oculta</p>
                                                            ) : null}
                                                        </div>
                                                        ...
                                                        <span className="shrink-0 rounded-full border border-white/5 bg-zinc-950/60 px-2 py-0.5 text-[11px] text-zinc-500">
                                                            {selectedCount
                                                                ...
                                                                : `${category.technologies.length} tecnologias`}
                                                        </span>
                                                    </div>
                                                </AccordionTrigger>
                                                    ...
                                                    <div className="grid gap-2">
                                                        {category.technologies.length ? (
                                                            category.technologies.map((technology) => (
                                                                <label
                                                                    key={technology.id}
                                                                    className="flex cursor-pointer items-center gap-3 rounded-lg border border-white/5 bg-zinc-950/30 px-3 py-2.5 text-sm text-zinc-300 transition-colors hover:border-violet-500/20 hover:bg-violet-500/5"
                                                                >
                                                                    <input
                                                                        name="technologyIds"
                                                                        type="checkbox"
                                                                    ...
                                                                    <span
                                                                        ...
                                                                        style={{ backgroundColor: technology.color ?? "#71717a" }}
                                                                    />
                                                            ...
                                                            <p className="py-2 text-xs text-zinc-600">Nenhuma tecnologia cadastrada.</p>
                                                        )}
                                                    </div>
                                                </AccordionContent>
                                            </AccordionItem>
...
export function ProjectFormHeading({ editing }: { editing: boolean }) {
    return (
        <div className="mb-6 flex items-start gap-3">
            ...
            <div>
                <h2 className="text-2xl font-semibold tracking-tight text-white">{editing ? "Editar projeto" : "Novo projeto"}</h2>
