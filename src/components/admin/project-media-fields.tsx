...
export function ProjectMediaFields({ mediaAssets, value, errors, onChange }: ProjectMediaFieldsProps) {
	...
	function updateItem(mediaId: string, field: "altPt" | "altEn", fieldValue: string,) {
		onChange(value.map((item) =>
			item.mediaId === mediaId ? { ...item, [field]: fieldValue } : item,
		));
	}
		...
		<Card className="w-full min-w-0 border-violet-500/10 bg-zinc-900/70 shadow-lg shadow-black/10 ring-0">
			<CardHeader className="border-b border-white/5">
				...
				<CardDescription className="text-zinc-500">
					Escolha a capa, monte a galeria e descreva as imagens nos dois idiomas.
				</CardDescription>
			</CardHeader>
						...
						<div className="space-y-3">
							<div>
								...
								<p className="mt-1 text-xs text-zinc-600">
									A capa não é repetida na galeria.
								</p>
							</div>
							...
							<div className="grid min-w-0 gap-2 md:grid-cols-2">
								{mediaAssets.map((asset) => {
									...
									return (
										<label key={asset.id} className="flex min-w-0 cursor-pointer items-center gap-3 rounded-lg border border-white/5 bg-zinc-950/30 px-3 py-2.5 text-sm text-zinc-300 transition-colors hover:border-violet-500/20 hover:bg-violet-500/5 has-disabled:cursor-not-allowed has-disabled:opacity-50">
											<input
												type="checkbox"
												checked={selected}
												disabled={isCover}
							...
							{gallery.length ? (
								<div className="space-y-3">
									{gallery.map((item, index) => {
										...
										return (
											<div key={item.mediaId} className="grid min-w-0 gap-3 rounded-xl border border-white/5 bg-white/2.5 p-3 md:grid-cols-[8rem_minmax(0,1fr)]">
												<div className="space-y-2">
													<MediaPreview asset={asset} />
													<div className="flex justify-center gap-1">
														<Button type="button" variant="ghost" size="icon-sm" disabled={index === 0} onClick={() => moveGallery(index, -1)} className="text-zinc-500 hover:text-violet-300">
															<ArrowUp />
															<span className="sr-only">Mover para cima</span>
														</Button>
														<Button type="button" variant="ghost" size="icon-sm" disabled={index === gallery.length - 1} onClick={() => moveGallery(index, 1)} className="text-zinc-500 hover:text-violet-300">
															<ArrowDown />
															<span className="sr-only">Mover para baixo</span>
														</Button>
														<Button type="button" variant="ghost" size="icon-sm" onClick={() => toggleGallery(item.mediaId, false)} className="text-zinc-500 hover:text-red-300">
															<X />
												...
												<div className="grid min-w-0 content-start gap-3">
													<div className="space-y-1.5">
														<Label htmlFor={`gallery-${item.mediaId}-alt-pt`}>
															Texto alternativo (PT)
														</Label>
														<Input
													...
													<div className="space-y-1.5">
														<Label htmlFor={`gallery-${item.mediaId}-alt-en`}>
															Alt text (EN)
														</Label>
														<Input
