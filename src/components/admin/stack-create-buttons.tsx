...
export function StackCreateButtons({ categoryOptions }: { categoryOptions: CategoryOption[]; }) {
	...
	return (
		<section className="flex flex-col gap-3 sm:flex-row justify-end">
				...
				<DialogContent className="border-white/10 bg-zinc-950 text-zinc-100 sm:max-w-lg">
					<DialogHeader>
						...
						<DialogDescription className="text-zinc-500">
							Agrupe as tecnologias por área de atuação.
						</DialogDescription>
					</DialogHeader>
			...
			<Dialog>
				<DialogTrigger asChild>
					<Button variant="outline" disabled={!hasCategories} className="border-violet-500/20 bg-violet-500/5 text-violet-200 hover:bg-violet-500/10 hover:text-violet-100">
						<PackagePlus />
						Nova tecnologia
					</Button>
				</DialogTrigger>
				...
				<DialogContent className="max-h-[90vh] overflow-y-auto border-white/10 bg-zinc-950 text-zinc-100 sm:max-w-xl">
					<DialogHeader>
						...
						<DialogDescription className="text-zinc-500">
							Cadastre uma stack dentro de uma categoria existente.
						</DialogDescription>
					</DialogHeader>
			...
			{!hasCategories ? (
				<p className="self-center text-xs text-zinc-500">
					Crie uma categoria antes de cadastrar tecnologias.
				</p>
			) : null}
		</section>
	);
