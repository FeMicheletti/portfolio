...
import { type AnalyticsEventDetails, type AnalyticsEventName } from "@/lib/analytics/types";
...
type AnalyticsContextValue = {
    track: (eventType: AnalyticsEventName, details?: AnalyticsEventDetails) => void;
};
    ...
    if (stored) {
        try {
            ...
            if (session.id && session.lastActivity && now - session.lastActivity < SESSION_TIMEOUT) {
                safeStorageSet(SESSION_KEY, JSON.stringify({ id: session.id, lastActivity: now }));
                return session.id;
            }
        } catch {
...
export function AnalyticsProvider({ locale, children }: { locale: Locale; children: ReactNode }) {
    const pageViewSent = useRef(false);
    const source = useMemo(() => (typeof window === "undefined" ? {} : attribution()), []);
...
export function usePortfolioAnalytics() {
    ...
    if (!context) throw new Error("usePortfolioAnalytics must be used inside AnalyticsProvider");
    return context;
}

type AnalyticsLinkProps = ComponentProps<"a"> & AnalyticsEventDetails & { eventType: AnalyticsEventName };

export function AnalyticsLink({ eventType, projectId, targetUrl, href, onClick, ...props }: AnalyticsLinkProps) {
    const { track } = usePortfolioAnalytics();
    const resolvedTarget = targetUrl ?? (typeof href === "string" ? href : undefined);
    return (
        <a
            href={href}
            onClick={(event) => {
                onClick?.(event);
                if (!event.defaultPrevented) track(eventType, { projectId, targetUrl: resolvedTarget });
            }}
            {...props}
        />
    );
...
export function AnalyticsProjectView({ projectId, children, ...props }: ComponentProps<"article"> & { projectId: string }) {
    const { track } = usePortfolioAnalytics();
