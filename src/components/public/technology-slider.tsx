		...
		<section aria-label={label} className="relative overflow-hidden border-y border-white/5 bg-zinc-950/80 py-5">
			...
			<div className="stack-marquee flex w-max items-center">
				{repeatedTechnologies.map((technology, index) => (
					<div key={`${technology.id}-${index}`} aria-hidden={index >= technologies.length} className="mx-2 flex shrink-0 items-center gap-3 rounded-full border border-white/10 bg-white/4 px-5 py-2.5 text-sm font-medium text-zinc-300">
