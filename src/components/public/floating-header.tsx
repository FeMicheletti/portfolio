...
export function FloatingHeader({ locale, navigation, timeZone }: { locale: Locale; navigation: NavigationLabels; timeZone: string }) {
    ...
    useEffect(() => {
        function updateTime() {
            ...
            const formattedTime = new Intl.DateTimeFormat(locale === "PT_BR" ? "pt-BR" : "en-US", {
                timeZone: timeZone,
                hour: "2-digit",
                minute: "2-digit",
                hour12: false,
        ...
        <header className="pointer-events-none fixed inset-x-0 top-0 z-50">
            <div className="relative mx-auto flex h-20 max-w-7xl items-center justify-between px-3 sm:px-6 lg:px-8">
                <div className="pointer-events-auto flex items-center gap-2 rounded-full border border-white/10 bg-zinc-950/75 px-3 py-2 text-xs font-medium text-zinc-300 shadow-xl shadow-black/30 backdrop-blur-xl">
                    ...
                    <span className="hidden sm:inline">{locale === "PT_BR" ? "São Paulo" : "São Paulo"}</span>
                    <span className="text-zinc-600">·</span>
                    <span className="tabular-nums text-white">{time || "--:--"}</span>
                    <span className="hidden text-zinc-500 sm:inline">BRT</span>
                </div>

                <nav
                    aria-label="Navegação principal"
                    className="pointer-events-auto absolute left-1/2 flex -translate-x-1/2 items-center gap-1 rounded-[1.4rem] border border-white/15 bg-zinc-950/75 p-1.5 shadow-2xl shadow-black/40 backdrop-blur-xl max-[360px]:left-3 max-[360px]:origin-left max-[360px]:translate-x-0 max-[360px]:scale-90"
                >
                    {navigationItems.map(({ id, icon: Icon }) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span
                                    className={`absolute inset-x-2 bottom-0 h-0.5 rounded-full bg-violet-400 transition-opacity ${active ? "opacity-100" : "opacity-0"}`}
                                />

                                <span className="pointer-events-none absolute top-full mt-3 hidden whitespace-nowrap rounded-lg border border-white/10 bg-zinc-800 px-3 py-1.5 text-xs font-medium text-white opacity-0 shadow-xl transition-all duration-200 group-hover:translate-y-1 group-hover:opacity-100 md:block">
                                    {label}
                                </span>
                            </a>
                        );
                ...
                <div className="pointer-events-auto flex items-center rounded-full border border-white/10 bg-zinc-950/75 p-1 text-xs font-medium shadow-xl shadow-black/30 backdrop-blur-xl">
                    <Link
                        href="/"
                        onClick={() => locale !== "PT_BR" && track("LANGUAGE_CHANGE", { targetUrl: "/" })}
                        aria-label="Português"
                        className={
                            locale === "PT_BR"
                                ? "rounded-full bg-violet-600 px-3 py-1.5 text-white"
                                : "px-3 py-1.5 text-zinc-500 transition-colors hover:text-white"
                        }
                    >
                        PT
                    </Link>
                    ...
                    <Link
                        href="/?lang=en"
                        onClick={() => locale !== "EN_US" && track("LANGUAGE_CHANGE", { targetUrl: "/?lang=en" })}
                        aria-label="English"
                        className={
                            locale === "EN_US"
                                ? "rounded-full bg-violet-600 px-3 py-1.5 text-white"
