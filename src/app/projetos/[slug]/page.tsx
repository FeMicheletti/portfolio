...
import {
  AnalyticsLink,
  AnalyticsProvider,
} from "@/components/public/analytics-provider";
        ...
        ? [
            {
              ...
              alt:
                (locale === "PT_BR" ? cover.altPt : cover.altEn) ||
                translation.title,
            },
          ]
        : undefined,
          ...
          <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-5 sm:px-6">
            ...
            <div className="flex gap-2 text-xs">
              <Link
                ...
                className={
                  locale === "PT_BR"
                    ? "rounded-full bg-violet-600 px-3 py-1.5"
        ...
        <main className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-20">
		  ...
		  {project.startedAt ? (
				<span className="inline-flex items-center gap-2 rounded-full border border-white/10 px-3 py-2 text-xs text-zinc-400 mt-4">
					...
					{dateLabel(project.startedAt, locale)} —{" "}
					{dateLabel(project.finishedAt, locale) || copy.present}
				</span>
				) : null}
          {project.media[0] ? (
            <div className="relative mt-2 aspect-video overflow-hidden rounded-3xl border border-white/10">
              <Image
                ...
                alt={
                  (locale === "PT_BR"
                    ? project.media[0].altPt
                    : project.media[0].altEn) || content.title
                }
                fill
                priority
                unoptimized
          ...
          <div className="mt-16 grid gap-6 lg:grid-cols-2">
            {sections.map(([title, body]) => (
              <section
                key={title}
                className="rounded-2xl border border-white/8 bg-zinc-900/50 p-6 sm:p-8"
              >
                ...
                {project.media.slice(1).map((media) => (
                  ...
                  >
                    <Image
                      ...
                      alt={
                        (locale === "PT_BR" ? media.altPt : media.altEn) ||
                        content.title
                      }
                      fill
                      unoptimized
