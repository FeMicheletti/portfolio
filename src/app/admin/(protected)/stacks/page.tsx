...
export default async function StacksPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
	...
	const categoryOptions = categories.map(({ id, namePt }) => ({ id, namePt }));
	...
	return (
		<div className="space-y-6">
			<section>
				...
				<p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-400">Organize categorias e tecnologias em português e inglês antes de vinculá-las aos projetos.</p>
			</section>
										...
										<div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
											<AccordionTrigger className="min-w-0 flex-1 py-0 text-left hover:no-underline">
												<div>
													<div className="flex flex-wrap items-center gap-2">
														...
														<Badge className={category.visible ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-300" : "border-zinc-500/20 bg-zinc-500/10 text-zinc-400"}>{category.visible ? "Visível" : "Oculta"}</Badge>
													</div>
											...
											<div className="flex shrink-0 items-center gap-1 self-end sm:self-auto">
												<form action={toggleCategoryVisibilityAction.bind(null, category.id, !category.visible)}>
													<Button type="submit" variant="ghost" size="icon-sm" className="text-zinc-500 hover:bg-violet-500/10 hover:text-violet-300">
												...
												<StackDeleteButton id={category.id} name={category.namePt} type="category" blocked={category.technologies.length > 0} />
											</div>
										</div>
									</CardHeader>
									...
									<AccordionContent className="pb-0 h-fit">
										<CardContent className="space-y-4 pt-6">
											...
											{category.technologies.length ? (
																...
																<div className="flex min-w-0 items-center gap-3">
																	...
																	<div className="min-w-0">
																		...
																		<p className="text-xs text-zinc-500">
																			{technology.slug}
																			{technology.iconKey ? ` · ícone ${technology.iconKey}` : ""} · {technology._count.projects} projetos
																		</p>
																	</div>
																</div>
																<div className="flex items-center justify-end gap-1">
																	<Badge className={technology.visible ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-300" : "border-zinc-500/20 bg-zinc-500/10 text-zinc-400"}>{technology.visible ? "Visível" : "Oculta"}</Badge>
																	<form action={toggleTechnologyVisibilityAction.bind(null, technology.id, !technology.visible)}>
																		<Button type="submit" variant="ghost" size="icon-sm" className="text-zinc-500 hover:bg-violet-500/10 hover:text-violet-300">
																	...
																	<StackDeleteButton id={technology.id} name={technology.name} type="technology" blocked={technology._count.projects > 0} />
																</div>
															</div>
															<details className="mt-3 rounded-lg border border-white/5 bg-zinc-950/30 p-3">
																<summary className="cursor-pointer list-none text-xs font-medium text-violet-300">Editar tecnologia</summary>
																<div className="pt-4">
																	<TechnologyForm
																		id={technology.id}
												...
												<p className="rounded-lg border border-dashed border-white/10 p-6 text-center text-sm text-zinc-500">Nenhuma tecnologia nesta categoria.</p>
											)}
										</CardContent>
									</AccordionContent>
								</AccordionItem>
