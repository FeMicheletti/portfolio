...
function parsePeriod(value?: string): AnalyticsPeriod {
  ...
  return analyticsPeriods.includes(parsed as AnalyticsPeriod)
    ...
    <div className="space-y-8">
      <section className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="text-2xl font-semibold tracking-tight text-white sm:text-3xl">
            Desempenho e interações
          </h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-400">
            Dados anônimos de navegação, origem, dispositivos e interesse nos
            projetos. Nenhum endereço IP é armazenado.
          </p>
        </div>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map(({ label, value, detail, icon: Icon }) => (
          <Card
            key={label}
            className="border-violet-500/10 bg-zinc-900/70 ring-0"
          >
        ...
        <Card className="border-violet-500/10 bg-zinc-900/70 ring-0">
          <CardHeader>
            ...
            <CardDescription className="text-zinc-500">
              Visualizações e visitantes únicos por dia.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-violet-500/10 bg-zinc-900/70 ring-0">
          ...
          <CardContent>
            {analytics.devices.length ? (
              <DistributionChart data={analytics.devices} />
            ) : (
              <EmptyState />
            )}
          </CardContent>
        ...
        <Card className="border-violet-500/10 bg-zinc-900/70 ring-0">
          <CardHeader>
            <CardTitle className="text-zinc-100">
              Interesse por projeto
            </CardTitle>
            <CardDescription className="text-zinc-500">
              Exposições, demos e acessos ao código.
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {analytics.projects.map((project) => (
                      ...
                      >
                        <td className="max-w-48 truncate py-3 text-zinc-200">
                          {project.name}
                        </td>
