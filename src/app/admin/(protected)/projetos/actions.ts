...
export async function createProjectAction(_state: ProjectFormState, formData: FormData): Promise<ProjectFormState> {
    ...
    if (!(await mediaIdsExist(result.data.projectMedia.map(({ mediaId }) => mediaId))))
        return { error: "Uma ou mais imagens selecionadas não existem.", fieldErrors: { projectMedia: ["Atualize a seleção de mídias."] } };
